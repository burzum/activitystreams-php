...
class SubscriptionService extends HttpResourceService
{
    ...
    public function deleteSubscription($stream_id, array $values = array())
    {
        ...
        $db_service->deleteTableRows(
            'subscriptions', 'object_id = ? AND stream_id = ?', array(
            $object->getId(),
            $stream->getId()
            )
        );
    ...
    public function postSubscription(array $values = array())
    {
        ...
        $db_service->deleteTableRows(
            'unsubscriptions', 'object_id = ? AND stream_id = ? AND application_id = ?', array(
            $object->getId(),
            $stream->getId(),
            $application_id
            )
